...
function App() {
  ...
  return (
    <>
      ...
      {status === 0 ? (
        <>
          <div className='main-title'><h1>CV-App: Fill the following fields</h1></div>    
          <GeneralInfo key='info' status={status} currentColor={currentColor} />
          <EducationExp key='edexp' status={status} currentColor={currentColor} />
          <PracticalExp key='prexp' status={status} currentColor={currentColor} />
          <Config key='cfg' status={status} setcurrentColor={setcurrentColor} currentColor={currentColor} />
